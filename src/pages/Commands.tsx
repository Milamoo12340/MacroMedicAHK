...
import { Plus, Terminal, Brain, Save, ToggleLeft, ToggleRight, Trash2 } from "lucide-react";
import SectionHeader from "@/components/features/SectionHeader";
import { mockCommands, mockTrainedResponses } from "@/lib/mockData";
...
export default function Commands() {
  ...
  const [commands, setCommands] = useState<BotCommand[]>(mockCommands);
  const [trained, setTrained] = useState<TrainedResponse[]>(mockTrainedResponses);
  ...
  return (
    <div className="space-y-8">
      <SectionHeader
        ...
        subtitle="Create slash commands for your ticket channels, and teach the AI exact phrasing for common issues."
      />
...
function CommandsPanel({
  ...
  commands: BotCommand[];
  setCommands: React.Dispatch<React.SetStateAction<BotCommand[]>>;
}) {
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-start gap-3 min-w-0 flex-1">
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-2 text-[13px] bg-background rounded-md border border-border p-3 font-mono text-muted-foreground">
                    {c.response}
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-1 shrink-0">
                ...
                <button className="btn-ghost h-8 px-2 hover:text-destructive" onClick={() => del(c.id)}>
...
function TrainingPanel({
  ...
  setTrained: React.Dispatch<React.SetStateAction<TrainedResponse[]>>;
}) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div key={r.id} className="card-surface p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                ...
                <div className="text-sm text-muted-foreground">{r.response}</div>
              </div>
              <button
                className="btn-ghost h-8 px-2 shrink-0"
                onClick={() =>
                  setTrained((p) => p.map((x) => (x.id === r.id ? { ...x, active: !x.active } : x)))
                }
              >
                {r.active ? <ToggleRight className="w-4 h-4 text-primary" /> : <ToggleLeft className="w-4 h-4" />}
              </button>
            </div>
          </div>
        ))}
      ...
      <div className="card-surface p-5 sticky top-20 self-start">
        <div className="font-mono text-[11px] tracking-widest uppercase text-primary mb-3">TRAIN NEW PATTERN</div>
        <div className="space-y-4">
          ...
          <Field label="Category">
            ...
            >
              {["Launch", "Resolution", "Admin Rights", "In-Game Position", "Debug", "Other"].map((c) => (
